...
const CATEGORIES = ['Semua', 'Finance', 'Technology', 'Real Estate', 'Consumer', 'Industrial']

const hkAssets = ASSETS.filter(a => a.class === 'hk')
const GAINERS = [...hkAssets].filter(a => a.change24h > 0).sort((a, b) => b.change24h - a.change24h)
const LOSERS = [...hkAssets].filter(a => a.change24h < 0).sort((a, b) => a.change24h - b.change24h)
const TOP_VOL = [...hkAssets].sort((a, b) => b.volume24h - a.volume24h)
...
export default function HKTab() {
  ...
  const preview = moversData.length > 0 ? moversData : hkAssets

  return (
    <div className="px-4 py-4 space-y-4 pb-20">
      {variant === 'option2' && <HKMarketDashboard />}
      {/* AI Market Pulse */}
      <MarketPulseCard data={MARKET_DATA.marketPulse.hk} />

      {/* Movers */}
      <div className="bg-white rounded-card shadow-card p-4">
        <p className="text-sm font-bold text-gray-900 mb-2">HK Stocks & ETF Movers</p>
...
function HKMarketDashboard() {
  const now = new Date()
  const wibHour = (now.getUTCHours() + 7) % 24
  const isOpen = wibHour >= 9 && wibHour < 16
  const CALENDAR = [
    { date: '19 Mar', event: 'Hong Kong CPI', impact: 'Medium', color: 'text-yellow-600 bg-yellow-50' },
    { date: '21 Mar', event: 'Caixin PMI Flash', impact: 'High', color: 'text-red-600 bg-red-50' },
    { date: '25 Mar', event: 'HK Trade Balance', impact: 'Medium', color: 'text-yellow-600 bg-yellow-50' },
  ]
  return (
    <div className="space-y-3">
      <div className="bg-white rounded-card shadow-card p-4 flex items-center justify-between">
        <div>
          <p className="text-xs font-bold text-gray-900">HKEX</p>
          <p className="text-xs text-gray-400 mt-0.5">Jam buka: 09:00 – 16:00 WIB</p>
        </div>
